...
// Fetch all suppliers and invoices
...
if (isset($_POST['add_ap'])) {
    ...
    if ($supplier_id && $po_id && $invoice_id && $amount && $due_date) {
        $query = "INSERT INTO accounts_payable (supplier_id, po_id, invoice_id, amount, due_date, status) 
                  VALUES ('{$supplier_id}', '{$po_id}', '{$invoice_id}', '{$amount}', '{$due_date}', 'Pending')";
...
<div class="row">
    ...
    <tbody>
        <?php
        $sql = "SELECT ap.ap_id, ap.amount, ap.due_date, ap.status, s.name AS supplier_name, i.invoice_number
